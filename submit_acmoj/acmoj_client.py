...
def main():
    ...
    # Submit git repository or C++ source file
    submit_parser = subparsers.add_parser("submit", help="Submit a git repository or C++ source file")
    submit_parser.add_argument("--problem-id", type=int, required=True, help="Problem ID")
    submit_parser.add_argument("--language", type=str, default="git",
    ...
    submit_parser.add_argument("--git-url", type=str, required=False,
                               help="Git repository URL (for git submissions)")
    ...
    if args.command == "submit":
        if args.language == "git":
            if not args.git_url:
                print("Error: --git-url is required for git submissions")
                exit(1)
            result = client.submit_git(args.problem_id, args.git_url)
        else:
            print("Error: Only git submissions are supported by this script")
            exit(1)
